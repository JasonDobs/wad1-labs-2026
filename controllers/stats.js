...
const stats = {
    createView(request, response) {
        ...
        let highestSongCount = playlists.length > 0
            ? Math.max(...playlists.map(playlist => playlist.songs.length))
            : 0;

        let mostSongPlaylists = playlists
            .filter(playlist => playlist.songs.length === highestSongCount)
        ...
        const statistics = {
            ...
            highestSongCount: highestSongCount,
            displayMostSongs: mostSongPlaylists
        };
